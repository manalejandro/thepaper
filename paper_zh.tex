...
然而，将我们与哪怕最近的恒星邻居分隔开来的距离，以任何人类尺度衡量都是令人咋舌的。半人马座$\alpha$星系位于 \SI{4.37}{光年} 之外——约 \SI{4.13e13}{\kilo\meter}。以旅行者1号约 \SI{17}{\kilo\meter\per\second} 的速度，这段旅程需要大约 \num{73000} 年。行星际旅行困难重重；恒星际旅行则几乎超出人类的想象。然而物理学并不禁止它。力学、热力学和电磁学定律——以巨大但有限的代价——允许向其他恒星发送载人或无人探测器，并使其在人类有生之年内抵达目的地。
...
\end{equation}
这个方程由齐奥尔科夫斯基于1903年首先发表，是火箭工程的基石。比冲 $I_{\mathrm{sp}} = v_e/g_0$（单位：秒）是任何推进系统性能的核心指标。
...
\end{equation}
随 $\Delta v$ 指数增长——这使得化学推进对于恒星际速度完全不可行。
...
奥利安计划提出在推进板后方引爆核弹产生冲量。理论比冲在 $10^4$ 至 $10^6$\,s 之间，可实现数周内的载人土星任务。
...
在接近光速时，牛顿力学失效。爱因斯坦狭义相对论（1905）引入洛伦兹因子：
\begin{equation}
...
\end{align}
其中 $\tau$ 为旅行者的固有时间。在 $a = g = \SI{9.81}{\meter\per\second\squared}$ 时：
\begin{itemize}[noitemsep]
  \item 比邻星（4.24光年）：固有时间约3.5年，坐标时间约5.9年。
  \item 银河系中心（26000光年）：旅行者仅老化21年，地球上却过了52026年。
  \item 仙女座星系（250万光年）：旅行者老化约28.4年，地球上却过了250万年。
\end{itemize}
...
\end{equation}
将 $10^6$\,kg 有效载荷加速到 $\beta=0.2$，需要能量约 $\SI{1.8e21}{\joule}$，
相当于全球约两个月的能源产量——这对卡尔达肖夫 II 型文明而言是可行的。
...
银河系中目前有多少技术活跃的文明能够进行星际通信？弗兰克·德雷克（1961）提出了基础估算：
\begin{equation}
...
若 $N$ 很大，费米的问题便无法回避：\textit{"他们在哪里？"}主要解释可归为三类：
\begin{enumerate}[noitemsep]
  \item \textbf{稀有性。} 生命或智慧极为罕见（"稀有地球"假说），$N \ll 1$。
  \item \textbf{大过滤器。} 存在某个极低概率的进化跃迁，或在我们之前，或在我们之后。
  \item \textbf{通信差异。} 先进文明存在，但以我们未曾关注的方式通信（动物园假说、黑暗森林理论）。
\end{enumerate}
...
任何足够先进的文明必定已经发现了相同的数学真理：2、3、5、7、11的素性；$\pi$ 的值；欧几里得几何的结构；以及自然界的物理常数。数学不是人类的发明，而是对物理现实中固有模式的发现——是任何掌握技术的智慧生命都能解读的唯一框架。

\subsection{林科斯方案与素数编码}

汉斯·弗罗伊登塔尔（1960）提出从零开始构建一种星际语言（林科斯）。编码层次如下：
\begin{enumerate}[noitemsep]
...
\end{pmatrix}
\end{equation}
若 $b > c$，则相互发信 $(S, S)$ 是纳什均衡。若 $b < 0$（接触有危险），则沉默成为双方的占优策略——即黑暗森林的博弈论解释。
